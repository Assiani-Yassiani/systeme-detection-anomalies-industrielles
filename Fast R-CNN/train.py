...
class myDataset(Dataset):
    def __init__(
        self, data_folder="../boxes_v2", dataset_file="../boxes_v2.txt", transform=None
    ):
        """
        input:
...
for epoch in range(NUM_EPOCHS):
    ...
    train_loss, train_accuracy = train_phase(
        train_loader=train_loader, model=model, criterion=criterion, optimizer=optimizer
    )
    print(
        "Train Loss: {:.4f}, Train Accuracy: {:.4f}%".format(train_loss, train_accuracy)
    )

    test_loss, test_accuracy = test_phase(
        test_loader=test_loader, model=model, criterion=criterion
    )
    print(
        "Valid Loss: {:.4f}, Valid Accuracy: {:.4f}%".format(test_loss, test_accuracy)
    )
